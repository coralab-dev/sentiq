...
const RESTAURANT_ERRORS: Record<string, string> = { invalid_payload: "Revisa los datos del restaurante y sucursal.", unauthorized: "Tu sesión expiró.", forbidden: "No tienes permiso para crear restaurantes.", slug_conflict: "Ya existe un restaurante con ese nombre.", server_error: "No pudimos crear el restaurante." };
const ADMIN_ERRORS: Record<string, string> = { invalid_payload: "Revisa nombre y email del administrador.", unauthorized: "Tu sesión expiró.", forbidden: "No tienes permiso para crear administradores.", restaurant_not_found: "El restaurante creado no está disponible.", email_conflict: "Ese email ya pertenece a otro usuario o rol.", admin_exists: "Este restaurante ya tiene administrador.", server_error: "No pudimos crear el administrador." };
...
export function validateNewRestaurantDraft(draft: NewRestaurantDraft): NewRestaurantErrors {
  ...
  for (const key of ["contactPhone", "branchInternalPhone"] as const) { const value = draft[key].trim(); if (value && !PHONE.test(value)) errors[key] = "Ingresa un teléfono válido."; }
  return errors;
}
